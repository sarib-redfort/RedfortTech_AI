...
export function Preloader({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    logger.debug("[Preloader] Mounted");
    ...
    if (!preloaderEl || !textEl || !glowEl) {
      logger.warn("[Preloader] Refs not ready — calling onComplete immediately");
      onCompleteRef.current();
      return;
    }

    logger.debug("[Preloader] Animation started");
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        logger.debug("[Preloader] Animation completed — onComplete fired");
        onCompleteRef.current();
      },
    });
    ...
    return () => {
      logger.debug("[Preloader] Cleanup — killing timeline");
      tl.kill();
    };
