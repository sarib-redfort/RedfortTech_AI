...
import BlogsView from './features/blogs/BlogsView';
import ServicesView from './features/services/ServicesView';
import IndustriesView from './features/industries/IndustriesView';
import CaseStudiesView from './features/case-studies/CaseStudiesView';
import TeamMembersView from './features/team/TeamMembersView';
import ContactsView from './features/contacts/ContactsView';
import TestimonialsView from './features/testimonials/TestimonialsView';
import CmsView from './features/content/CmsView';
...
export default function App() {
  ...
  const [homepage, setHomepage] = useState<HomepageCms>(EMPTY_HOMEPAGE);
  const [about, setAbout] = useState<AboutCms>(EMPTY_ABOUT);
  ...
  const [deleteModalState, setDeleteModalState] = useState<{
    ...
    itemName: string;
    targetId: string;
    type: 'user' | 'blog' | 'service' | 'testimonial' | 'case_study' | 'team_member' | null;
  }>({
    isOpen: false,
    itemName: '',
    targetId: '',
  ...
  const handleAddBlog = async (b: Omit<Blog, 'id' | 'publishedAt'>) => {
    try {
      ...
      logger.error('[APP] Failed to create blog:', error);
      throw error;
    }
  };
  ...
  const handleEditBlog = async (id: string, updatedFields: Partial<Blog>) => {
    try {
      ...
      logger.error('[APP] Failed to update blog:', error);
      throw error;
    }
  };
  ...
  const handleAddIndustry = async (payload: FormData) => {
    try {
      ...
      logger.error('[APP] Failed to create industry:', error);
      throw error;
    }
  };
  const handleEditIndustry = async (id: string, payload: FormData) => {
    try {
      ...
      logger.error('[APP] Failed to update industry:', error);
      throw error;
    }
  };
  ...
  const handleAddTeamMember = async (payload: FormData) => {
    try {
      ...
      logger.error('[APP] Failed to create team member:', error);
      throw error;
    }
  };
  const handleEditTeamMember = async (id: string, payload: FormData) => {
    try {
      ...
      logger.error('[APP] Failed to update team member:', error);
      throw error;
    }
  };
  ...
  const handleAddTestimonial = async (payload: FormData) => {
    try {
      ...
      logger.error('[APP] Failed to create testimonial:', error);
      throw error;
    }
  };
  const handleEditTestimonial = async (id: string, payload: FormData) => {
    try {
      ...
      logger.error('[APP] Failed to update testimonial:', error);
      throw error;
    }
  };
  ...
  const handleAddFaq = async (f: Omit<Faq, 'id'>) => {
    try {
      ...
      logger.error('[APP] Failed to create FAQ:', error);
      throw error;
    }
  };
  const handleEditFaq = async (id: string, updatedFields: Partial<Faq>) => {
    try {
      ...
      logger.error('[APP] Failed to update FAQ:', error);
      throw error;
    }
  };
  ...
  const handleTabChange = (tab: string) => {
    ...
    logger.debug('[APP] Opening tab:', tab);
    if (tab === 'users') {
      logger.debug('[APP] Loading users from backend because Users tab was opened');
      void loadUsers();
    }
    if (tab === 'blogs') {
      logger.debug('[APP] Loading blogs from backend because Blogs tab was opened');
      void loadBlogs();
    }
    setActiveTab(tab);
